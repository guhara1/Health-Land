...
function homePage() {
  ...
  <section class="section section-alt"><div class="container prose">
    <h2>출장마사지·홈타이란</h2>
    <p>출장마사지는 관리사가 집·숙소·사무실 등 원하는 장소로 방문해 관리를 진행하는 이용 방식입니다. 매장을 찾아갈 필요가 없어 이동이 번거롭거나 익숙한 공간에서 편하게 받고 싶은 분들이 주로 선택합니다. ‘홈타이’는 집에서 받는 타이식 방문 관리를 가리키는 표현으로, 큰 틀에서는 출장마사지의 한 형태입니다. 헬스랜드는 이런 방문형 관리를 지역·프로그램·이용 방식별로 비교할 수 있도록 정리해 안내합니다.</p>
    <h2>헬스랜드는 이렇게 안내합니다</h2>
    <p>자극적인 추천이나 광고 문구 대신, 예약 전 실제로 확인해야 할 기준을 먼저 정리합니다. <a href="/region/">지역별 찾기</a>와 <a href="/subway/">지하철역별 찾기</a>로 가까운 곳을 좁혀 가고, <a href="/program/">마사지 프로그램</a>에서 스웨디시·타이마사지·아로마·홈타이를 비교한 뒤, <a href="/guide/">예약 가이드</a>의 순서대로 확인하면 처음 이용하는 분도 어렵지 않게 진행할 수 있습니다. 가격·운영 정보는 변동될 수 있으므로 예약 전 직접 확인하는 것을 권장합니다.</p>
  </div></section>
...
function aboutPage() {
  return simplePage({
    ...
    sections: `
      ...
      <p>출장마사지·홈타이는 가격과 이용 조건이 업체·지역·시간대에 따라 달라지기 쉬운 분야입니다. 그래서 헬스랜드는 특정 업체를 단정적으로 추천하기보다, 이용자가 스스로 판단할 수 있도록 ‘무엇을 확인해야 하는지’를 일관된 기준으로 정리합니다. 방문 권역, 코스 시간, 총 비용과 추가 요금, 위생·준비물처럼 예약 전에 체크하면 좋은 항목을 각 페이지에 반복해 안내하는 이유입니다.</p>

      <h2>이용 시 유의사항</h2>
      <p>본 사이트의 안내는 일반적인 이용 기준을 정리한 참고 자료이며, 개별 업체의 운영 방침·가격·방문 권역은 수시로 바뀔 수 있습니다. 따라서 어떤 페이지의 정보든 ‘예약 시점에 직접 확인’하는 것을 기본으로 삼아 주시기 바랍니다. 또한 헬스랜드는 건전한 관리 서비스 정보만을 다루며, 법령에 위배되는 이용을 안내하지 않습니다. 정확하지 않은 정보를 발견하시면 언제든 ${esc(site.email)}로 알려 주시면 확인 후 반영하겠습니다.</p>
...
function privacyPage() {
  return simplePage({
    ...
    eyebrow: "개인정보처리방침",
    h1: "개인정보처리방침",
    desc: "헬스랜드가 수집·이용하는 정보의 범위와 처리 방침, 이용자의 권리를 안내합니다.",
    sections: `
    ...
    faqs: [
      { q: "회원가입을 해야 하나요?", a: "아니요. 헬스랜드는 회원가입 없이 정보를 제공하며, 웹사이트 이용만으로 개인정보를 수집하지 않습니다." },
...
function termsPage() {
  return simplePage({
    ...
    eyebrow: "이용약관",
    h1: "이용약관",
    desc: "헬스랜드 웹사이트 이용 조건과 정보의 성격, 책임 범위를 안내합니다.",
    sections: `
    ...
    faqs: [
      { q: "헬스랜드가 직접 예약을 진행하나요?", a: "헬스랜드는 정보 안내 플랫폼이며, 실제 예약·이용 계약은 이용자와 해당 업체 사이에 성립합니다." },
