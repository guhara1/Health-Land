// 수도권 지하철 노선/역 페이지 생성기
// - /subway/ (노선 인덱스) → /subway/line/{노선}/ (노선) → /subway/{역}/ (역 정규 페이지)
// - 역은 환승 중복을 피해 역당 1개 정규 페이지(여러 노선/인접역/환승 정보 표시)
// - 역별 고유 정보(노선·인접역·환승) + 문장/메타 변형으로 도어웨이 방지
...
// 역 타이틀·디스크립션 변형 (역명 고유 → 중복 방지)
function stationMeta(station, lineNames) {
  const s = seed("st|" + station);
  const l1 = lineNames[0] || "수도권 전철";
  const titles = [
    `${station} 출장마사지·홈타이 이용 안내 | ${site.name}`,
    `${station} 인근 출장마사지·홈타이 — 지하철역 안내 | ${site.name}`,
    `${station} 홈타이·출장마사지 방문 예약 안내 | ${site.name}`,
    `${station} 출장마사지 예약 가이드 (지하철역) | ${site.name}`,
  ];
  const descs = [
    `${station}(${l1}) 인근 출장마사지·홈타이 방문 권역과 예약 확인 사항 안내.`,
    `${station}에서 출장마사지·홈타이를 찾을 때 볼 방문 권역과 예약 기준 안내.`,
    `${station} 인근 홈타이·출장마사지 이용 흐름과 비용·예약 방법 안내.`,
    `${station} 출장마사지 예약 체크리스트와 인접역 정보를 정리했습니다.`,
    `지하철 ${station} 기준 출장마사지·홈타이 방문·예약 안내입니다.`,
  ];
  return { title: titles[s % titles.length], description: descs[(s >>> 2) % descs.length].slice(0, 80) };
}

const crumb = (parts) => parts.map(([name, url]) => (url ? { name, url } : { name, url: "" }));

// ---------- 역 페이지(정규) ----------
function stationPage(reg, lineBySlug) {
  const station = reg.name;
  const s = seed("S" + station);
  ...
  for (const l of reg.lines) {
    const line = lineBySlug[l.lineSlug];
    const prev = line.stations[l.idx - 1];
    const next = line.stations[l.idx + 1];
    if (prev) neigh.push(prev);
    if (next) neigh.push(next);
  }
  const uniqNeigh = [...new Set(neigh)].slice(0, 6);
  const neighText = uniqNeigh.length ? uniqNeigh.map((n) => n).join("·") : "인근 역";
  ...
  const transferLine = isTransfer
    ? `${station}은(는) ${lineNames.length}개 노선이 만나는 환승역이라 여러 방향에서 접근이 가능합니다. `
    : "";
  ...
  const neighLinks = uniqNeigh.map((n) => `<a href="/subway/${stSlug(n)}/">${esc(n)}</a>`).join("");

  const body = `
  <nav class="breadcrumb container" aria-label="위치">
    <a href="/">홈</a><span>›</span><a href="/subway/">지하철역별 찾기</a><span>›</span><a href="/subway/line/${reg.lines[0].lineSlug}/">${esc(reg.lines[0].lineName)}</a><span>›</span>${esc(station)}
  </nav>
  <article class="section-tight"><div class="container prose">
    <p class="card-tag" style="color:var(--color-accent);font-weight:700">지하철 ${esc(lineText)}</p>
  ...
  return {
    ...
    html: layout({
      ...stationMeta(station, lineNames),
      path,
      body,
      structuredData: [
        faqLd(faqs),
        articleLd({ headline: `${station} 출장마사지·홈타이 이용 안내`, description: `${station} 인근 출장마사지·홈타이 이용 안내`, path, modified: MODIFIED }),
        pricingLd(),
      ],
      breadcrumb: crumb([["홈", "/"], ["지하철역별 찾기", "/subway/"], [reg.lines[0].lineName, `/subway/line/${reg.lines[0].lineSlug}/`], [station, path]]),
    }),
  };
}
...
function linePage(line) {
  const stationLinks = line.stations.map((st) => `<a href="/subway/${stSlug(st)}/">${esc(st)}</a>`).join("");
  const area = line.area || "수도권";
  const faqs = [
  ...
  <article class="section-tight"><div class="container prose">
    <p class="card-tag" style="color:var(--color-accent);font-weight:700">수도권 지하철</p>
    ...
    <p>매장 이용이 시설과 부대 서비스를 함께 쓰는 방식이라면, 홈타이는 역 인근 집·숙소로 관리사가 찾아오는 방문 방식입니다. 이동 부담이 적고 관리 후 바로 쉴 수 있는 대신, 관리 공간과 타월 등 준비물을 직접 챙겨야 하므로 예약 시 준비 사항을 확인하는 것이 좋습니다.</p>
    ...
    <div class="link-cloud">
      <a href="/region/seoul/">서울</a><a href="/region/gyeonggi/">경기</a><a href="/region/incheon/">인천</a>
      <a href="/program/swedish/">스웨디시</a><a href="/program/home-care/">홈타이</a><a href="/guide/">예약 가이드</a>
    </div>
...
// ---------- 지하철 인덱스 ----------
function subwayIndex(lines, stationCount) {
  const cards = lines.map((l) => `<a class="card" href="/subway/line/${l.slug}/">
      <h3>${esc(l.name)}</h3>
      <p>${esc(l.area || "수도권")} · ${l.stations.length}개 역</p>
    </a>`).join("\n      ");
  const body = `
  <section class="hero"><div class="container">
    <p class="eyebrow">지하철역별 찾기</p>
    <h1>수도권 지하철역 출장마사지·홈타이 찾기</h1>
    <p>서울·경기·인천 지하철 노선과 역 기준으로 출장마사지·홈타이 이용 안내를 확인할 수 있습니다. 노선을 고른 뒤 가까운 역을 선택하면 역세권 안내를 볼 수 있습니다. (총 ${lines.length}개 노선·${stationCount}개 역)</p>
  ...
  <section class="section"><div class="container">
    <div class="section-head"><span class="eyebrow">노선 선택</span><h2>노선을 고르세요</h2></div>
    <div class="grid grid-3">${cards}</div>
  </div></section>
  <section class="section section-alt"><div class="container prose">
    <h2>지하철역 기준으로 찾는 방법</h2>
    <p>서울·경기·인천 지하철은 노선마다 지나는 지역과 분위기가 다릅니다. 노선을 먼저 고른 뒤 가까운 역을 선택하면, 해당 역세권 기준으로 출장마사지·홈타이 방문 권역과 예약 안내를 확인할 수 있습니다. 같은 역이라도 출발지에 따라 방문 소요 시간이 달라지므로, 원하는 역과 시간대를 정해 두면 안내가 빠릅니다.</p>
  ...
  return {
    ...
    html: layout({
      title: `수도권 지하철역 출장마사지·홈타이 찾기 | ${site.name}`,
      description: "서울·경기·인천 지하철 노선·역별 출장마사지·홈타이 이용 안내를 확인하세요.",
      path: "/subway/",
      body,
...
// 역 슬러그(정규) — 모듈 전역에서 동일 규칙 사용
let _slugMap = null;
function stSlug(name) { return _slugMap ? _slugMap.get(name) : slugify(name.replace(/역$/, "")); }

export function buildSubwayPages(lines) {
  // 노선 슬러그
  for (const l of lines) l.slug = l.slug || slugify(l.name);
  ...
  for (const line of lines) {
    line.stations.forEach((st, idx) => {
      let r = reg.get(st);
      if (!r) { r = { name: st, lines: [] }; reg.set(st, r); }
  ...
  for (const r of reg.values()) {
    ...
    used.add(sg);
    r.slug = sg;
    _slugMap.set(r.name, sg);
  }

  const pages = [subwayIndex(lines, reg.size)];
  for (const line of lines) pages.push(linePage(line));
  for (const r of reg.values()) pages.push(stationPage(r, lineBySlug));
  return pages;
}
